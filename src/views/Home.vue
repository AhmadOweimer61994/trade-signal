<template>
  <div class="p-4 space-y-3">
    <div class="flex items-center gap-2">
      <input
        v-model="target"
        placeholder="اسم القناة أو @username"
        class="border rounded px-3 py-1.5 w-80"
      />
      <button @click="listen" class="border rounded px-3 py-1.5">استمع</button>
      <button @click="listTargets" class="border rounded px-3 py-1.5">
        عرض الأهداف
      </button>
    </div>

    <div class="text-sm text-gray-600">
      الأهداف الحالية: {{ targets.join(", ") || "—" }}
    </div>
    ...
    <div v-if="login.needCode" class="p-2 bg-amber-50 border rounded">
      ...
      <button @click="sendCode" class="border rounded px-2 py-1">
        إرسال الكود
      </button>
    </div>
    <div v-if="login.needPassword" class="p-2 bg-amber-50 border rounded">
      ...
      />
      <button @click="sendPassword" class="border rounded px-2 py-1">
        إرسال
      </button>
    </div>
  </div>
